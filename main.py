...
if fixed_analysis_flag == 1:
	...
	outdirname = 'shared_power_pv_{}_battery_{}'.format(pv_size, battery_size)
	#external folder
	try: Path.mkdir(basepath / dirname / subdirname / subsubdirname)
	except Exception: pass
	#Inner folder
	outpath = basepath / dirname / subdirname / subsubdirname / outdirname
	try: Path.mkdir(outpath)
	except Exception: pass
	
	#Store values: 
	sp_weekday = pd.DataFrame(shared_power_month_day[:,:,0])
	sp_weekend = pd.DataFrame(shared_power_month_day[:,:,1])
	#Write in raw format
	sp_weekday.to_csv(outpath / 'wd.csv',
				      sep = ";",
					  decimal = ",",
					  index = False)
